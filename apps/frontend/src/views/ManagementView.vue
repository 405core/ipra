    ...
    <section class="admin-content">
      <section v-if="activeTab === 'profiles'" class="admin-panel">
        ...
        <p class="admin-filter-summary">
          当前筛选后 {{ filteredProfiles.length }} 条基础画像。
        </p>
        ...
        <div class="admin-table">
          ...
          >
            <div class="admin-row__profile-content">
              ...
              <div
                v-if="
                  buildProfileRiskTags(item).length ||
                  buildProfileNotes(item).length
                "
      ...
      <section v-else-if="activeTab === 'watchlist'" class="admin-panel">
        ...
        <p class="admin-filter-summary">
          当前筛选后 {{ filteredWatchlist.length }} 条高风险名单。
        </p>
      ...
      <section v-else class="admin-panel">
        <template v-if="activeTab === 'audit'">
          ...
          <p class="admin-filter-summary">
            当前筛选后 {{ filteredAuditLogs.length }} 条审计日志。
          </p>
            ...
            >
              <div class="admin-audit-item__head">
                ...
                <span
                  class="admin-row__status"
                  :class="
                    item.result === 'success' ? 'is-active' : 'is-disabled'
                  "
                >
                  <span class="admin-row__status-dot"></span>
              ...
              <div class="admin-audit-item__meta">
                ...
                <span
                  >{{ item.actorName || '未知操作人' }} /
                  {{ item.actorWorkId || '-' }}</span
                >
        ...
        <template v-else>
          ...
          <p class="admin-filter-summary">
            当前筛选后 {{ filteredUsers.length }} 个用户。
          </p>
            ...
            <table class="admin-user-table">
              ...
              <tbody>
                <tr v-for="item in filteredUsers" :key="item.id">
                  <td>{{ item.workId }}</td>
                  <td>
                    <strong class="admin-user-table__name">{{
                      item.name
                    }}</strong>
                  </td>
                  <td>
                  ...
                  <td>
                    <span
                      class="admin-row__status"
                      :class="
                        item.status === 'active' ? 'is-active' : 'is-disabled'
                      "
                    >
                      <span class="admin-row__status-dot"></span>
                      {{ formatUserStatusLabel(item.status) }}
