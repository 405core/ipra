...
该模块负责完成离线数据接入、画像入库、高风险名单入库和现场证件匹配，是后续大模型预评估的上下文基础。

```mermaid
flowchart TD
    A["检查员上传 Excel / CSV 文件"] --> B["后端校验文件格式与字段完整性"]
    B --> C["按导入类型解析基础画像或高风险名单"]
    C --> D["分别写入 passenger_profile / high_risk_watchlist"]
    D --> E["建立证件号索引"]
    E --> F["前端扫描或输入证件号"]
    F --> G["服务层合并查询画像与名单"]
    G --> H{"是否命中高风险名单"}
...
#### 4.1.1 画像数据离线导入（对应 `2.1.1`）

系统基于 `import_batch_log`、`passenger_profile` 和 `high_risk_watchlist` 实现离线导入。检查员通过内网前端上传采购方提供的 Excel 或 CSV 文件后，后端先登记导入批次，再按导入类型分别校验和写入。

对缺失主键、格式错误、重复行等异常数据，系统按行记录失败原因并返回导入结果摘要。基础画像导入只写 `passenger_profile`；高风险名单导入只写 `high_risk_watchlist`。

#### 4.1.2 全息画像维度落地（对应 `2.1.2`）

为保持结构简洁，`passenger_profile` 只保留证件号码、姓名和 `profile_data`。其中 `profile_data` 统一承载画像 JSON，覆盖个人基本信息、订票与行程信息、历史出行记录、职业背景以及违法犯罪记录等维度。

高风险名单单独存放在 `high_risk_watchlist`，查询时按 `document_num` 与画像做服务层合并。这样既满足证件快速匹配和高风险显著预警，也支持“名单已导入但基础画像尚未导入”的场景。
