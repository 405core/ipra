...
系统基于现有 `import_batch_log` 和 `passenger_profile` 实现离线导入。检查员通过内网前端上传采购方提供的 Excel 或 CSV 文件后，后端先登记导入批次，再校验证件类型、证件号码、姓名等核心字段，并按 `(document_type, document_num, issuing_region)` 完成画像写入或更新。

对缺失主键、格式错误、无效日期、重复行等异常数据，系统按行记录失败原因并返回导入结果摘要；高风险名单导入时同步更新 `is_high_risk` 和相关风险标签，保证后续证件匹配时能够直接触发前端预警。

#### 4.1.2 全息画像维度落地（对应 `2.1.2`）

基于现有 `passenger_profile` 宽表结构，证件类型、证件号码、签发地区、姓名、性别、出生日期和高风险标识等稳定字段直接存放在主表列中；不同证件的差异化信息存放在 `identity_details`，其余画像内容统一存放在 `dimension_data`。

`dimension_data` 需覆盖个人基本信息、订票与行程信息、历史出行记录、职业背景以及违法犯罪记录等维度，既满足前端全息画像展示，也为后续证件快速匹配和大模型问询策略生成提供完整上下文。
